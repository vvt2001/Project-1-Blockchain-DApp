...
//Helper function to quickly get the debtor's CREDITOR address AKA their neighbors
...
//Helper function to find the smallest weight in a cycle
async function findTheLeastAmount(path) {
    let amount = 0;
    ...
    for (let i = 1; i < path.length; i++) {
        ...
        for (let j = 0; j < result.length; j++) {
            ...
            if (result[j].owingAddress == path[i]) {
                //auto put the first node's weight to track
                if (i == 1 || Number(result[j].amount) < amount) {
...
async function add_IOU(creditor, amount) {
    ...
    await BlockchainSplitwise.add_IOU(defaultAccount, creditor, amount);
    ...
    const leastAmount = await findTheLeastAmount(cyclePath);
    ...
    for (let i = 1; i < cyclePath.length; i++) {
        ...
        await BlockchainSplitwise.add_IOU(cyclePath[i], cyclePath[i - 1], leastAmount);
    }
    await BlockchainSplitwise.add_IOU(cyclePath[0], cyclePath[cyclePath.length - 1], leastAmount);
}
